-- Store validated identity fields for authorized staff views; retain hashed sensitive columns for verification.
...
declare
  ...
  v_nin text := nullif(trim(p_payload ->> 'nin'), '');
...
begin
  ...
  if v_nin is null or v_nin !~ '^[0-9]{18}$' then raise exception using errcode = '22023', message = 'invalid national id'; end if;
  ...
  loop
    ...
    begin
      insert into public.complaints (tracking_id, citizen_name, citizen_nin, phone_encrypted, citizen_phone, national_id_encrypted, category, municipality, daira, neighborhood, subject, description, status, priority, deadline, pin_hash, created_at, updated_at)
      values (v_tracking_id, v_name, v_nin, encode(digest(v_phone, 'sha256'), 'hex'), v_phone, encode(digest(v_nin, 'sha256'), 'hex'), v_category, v_municipality, coalesce(v_daira, 'الوادي'), v_neighborhood, v_subject, v_description, 'جديد', 'عادي', v_created_at + interval '15 days', encode(digest(v_secret_pin, 'sha256'), 'hex'), v_created_at, v_created_at);
      exit;
